...
SeaLevelComponent <- setRefClass(
  ...
  methods = list(
    initialize = function(country_abrev, study_period, reference_period) {
      ...
      .self$directory <- paste0("./inst/generateddata/sealevel_data_", country_abrev)
    ...
    load_data = function() {
      ...
      for (filename in list.files(.self$directory, pattern = "\\.txt$")) {
        ...
        temp_data <- data.table::fread(file_path, sep = ";", select = 1:2, col.names = c("Date", paste0("Measurement_", sub("\\.txt$", "", filename))))
        temp_data$Date <- as.numeric(temp_data$Date)
        data.table::setkey(temp_data, Date)
        dataframes[[length(dataframes) + 1]] <- temp_data
      }
      combined_data <- Reduce(function(x, y) base::merge(x, y, by = "Date", all = TRUE), dataframes)
    ...
    correct_date_format = function(data) {
      ...
      data.table::setkey(data, Corrected_Date)
      return(data)
    },
    ...
    clean_data = function(data) {
      ...
      for (col in names(data)) {
        data.table::set(data, j = col, value = fifelse(data[[col]] == -99999, NA, data[[col]]))
      }
      return(data)
    },
    ...
    compute_monthly_stats = function(data, reference_period, stats) {
      ...
      monthly_data <- reference_data[, lapply(.SD, function(x) if (stats == "means") mean(x, na.rm = TRUE) else sd(x, na.rm = TRUE)),
                                     by = .(month = lubridate::month(Corrected_Date)), .SDcols = measurement_cols]

      if (stats == "means") {
        data.table::setnames(monthly_data, old = names(monthly_data)[-1], new = paste0("mean_", names(monthly_data)[-1]))
      } else if (stats == "std") {
        data.table::setnames(monthly_data, old = names(monthly_data)[-1], new = paste0("std_", names(monthly_data)[-1]))
      }
    ...
    standardize_data = function(data, monthly_means, monthly_std_devs, study_period) {
      ...
      study_data[, month := lubridate::month(Corrected_Date)]

      # Merge monthly means and standard deviations with study data
      study_data <- data.table::merge(study_data, monthly_means, by = "month", all.x = TRUE)
      study_data <- data.table::merge(study_data, monthly_std_devs, by = "month", all.x = TRUE)
    ...
    plot_rolling_mean = function(data, window = 60) {
      ...
      #'
      data$mean_value <- base::rowMeans(standardized_data[, -1, with = FALSE], na.rm = TRUE)
      ggplot2::ggplot(data, aes(x = Corrected_Date, y = mean_value)) +
        ggplot2::geom_line() +
        ggplot2::labs(title = "Mean Value of Sea Level Data", x = "Date", y = "Mean Value (Standardized)") +
    ...
    convert_to_xarray = function(data) {
      ...
      data_array <- data.table::as.data.table(data)
    ...
    resample_data = function(data) {
      ...
      resampled_data <- data[, lapply(.SD, mean, na.rm = TRUE),
                             by = .(Corrected_Date = lubridate::floor_date(Corrected_Date, "3 months"))]
