...
DroughtComponent <- setRefClass(
  ...
  methods = list(
    ...
    drought_interpolate = function() {
      ...
      years <- unique(year(.self$max_dry_days_dt$time_day))
      
      for (i in seq_along(years)[-length(years)]) {
        year_k <- .self$max_dry_days_dt[year(time_day) == years[i]]
        year_k_plus_1 <- .self$max_dry_days_dt[year(time_day) == years[i + 1]]
        ...
        for (month in 1:12) {
          ...
          interpolated_values[[length(interpolated_values) + 1]] <- data.table(
      ...
      last_year <- .self$max_dry_days_dt[year(time_day) == tail(years, 1)]
      for (month in 1:12) {
      ...
      interpolated_dt <- rbindlist(interpolated_values, fill = TRUE)
      
      # Set the order of data to ensure consistency
      setorder(interpolated_dt, longitude, latitude, time)
    ...
    standardize_metric = function(reference_period = NULL) {
      ...
      #'
      #' @param reference_period A character vector specifying the start and end of the reference period. If NULL, the object's 
    ...
    plot_anomaly_map = function(month_year) {
      ...
      ggplot(plot_data, aes(x = longitude, y = latitude, fill = standardized_dry_days)) +
        geom_tile() +
        scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
        labs(title = paste("Standardized Drought Anomaly Map for", month_year),
