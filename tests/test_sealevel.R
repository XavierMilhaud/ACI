...
test_that("SeaLevelComponent: standardize_sealevel method works against precomputed reference anomalies", {
  ...
  for (test_case in test_cases) {
    ...
    sea_level_data[, Corrected_Date := as.Date(Corrected_Date)]
    
    sea_level_component <- SeaLevelComponent$new("FRA", study_period_bis, reference_period_bis)
    ...
    reference_anomalies[, Corrected_Date := as.Date(Corrected_Date)]
    
    combined_df <- merge(
      data.table(time = calculated_anomalies$Corrected_Date, calculated_mean = rowMeans(calculated_anomalies[, -1, with = FALSE], na.rm = TRUE)),
      data.table(time = reference_anomalies$Corrected_Date, reference_mean = rowMeans(reference_anomalies[, -1, with = FALSE], na.rm = TRUE)),
      by = "time"
    )
    ...
    expect_equal(combined_df$calculated_mean, combined_df$reference_mean, tolerance = 0.1)
  }
})
